...
namespace DAL
{
    public class Comment
    {
        ...
        public int GetWoekCount(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int Add(Model.Comment user)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert intoComment(commentID,workId,userName,CommentContent,CommentTime)"+ " values(@CommentID,@WORKiD,@USERnAME,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENTITY";
